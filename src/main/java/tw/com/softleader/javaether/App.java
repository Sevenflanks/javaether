package tw.com.softleader.javaether;

import java.nio.charset.Charset;

import org.ethereum.core.CallTransaction;
import org.ethereum.crypto.ECKey;
import org.ethereum.crypto.SHA3Helper;
import org.ethereum.facade.EthereumFactory;
import org.ethereum.solidity.Abi;
import org.spongycastle.util.encoders.Hex;

import tw.com.softleader.javaether.adapter.HelloWorldAdapter;

/**
 * Hello world!
 *
 */
public class App {
  private static final byte[] senderPrivateKey = SHA3Helper.sha3("cow".getBytes()); // cd2a3d9f938e13cd947ec05abc7fe734df8dd826
  private static final ECKey ecKey = ECKey.fromPrivate(senderPrivateKey);
  
  public static void main(String[] args) throws Exception {
    
//    EthereumFactory.createEthereum(HelloWorldAdapter.Config.class);
//    
//    String toAddress = "56033Cf6B4Ff05ce63e48420d3A6142a593473c0";
//    CallTransaction.Function function = CallTransaction.Function.fromSignature("deposit", "bytes32");
//    TxPackage tx = new TxPackage(toAddress, ecKey, function, 1_000_000_000_000_000_000L, "bbb");
//    
//    HelloWorldAdapter.txs.add(tx);
    
    
    String data = "2df69da951842023d18150844de1acd56d0c0ce6898b6bbbf6b8c3118df2b7e0";
    
    byte[] hex = Hex.decode(data);
    System.out.println(new String(hex, Charset.forName("ASCII")));
  }
  
}
